...
function WheelMenuMixin:LayoutOptions()
    ...
    for index = 1, optionCount do
        ...
        if button then
            local angle = -pi * 0.5 + (index - 1) * sliceAngle
...
function WheelMenuMixin:GetOptionIndexFromCursor(cursorX, cursorY)
    ...
    local angle = normalizeAngle(atan2(deltaY, deltaX) + pi * 0.5)
    local sliceAngle = threeSixty / optionCount
    local index = math.floor(angle / sliceAngle) + 1
    if index < 1 then
        index = 1
...
function WheelMenuMixin:RefreshHoverFromCursor()
    local cursorX, cursorY = detailsFramework:GetCursorPosition()
    local hoveredIndex = self:GetOptionIndexFromCursor(cursorX, cursorY)
...
function WheelMenuMixin:OpenAtCursor()
    local cursorX, cursorY = detailsFramework:GetCursorPosition()
    self:ClearAllPoints()
    self:SetPoint("CENTER", UIParent, "BOTTOMLEFT", cursorX, cursorY)
    self:SetHoveredIndex(nil)
    self:Show()
...
function detailsFramework:CreateWheelMenu(name, parent, config)
    config = config or {}
    ...
    menu:SetFrameStrata(config.frameStrata or "FULLSCREEN_DIALOG")
